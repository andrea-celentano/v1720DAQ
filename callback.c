...
void configure_enable_callback(GtkWidget *window, gpointer data)
{
...
#endif
  samples_per_channel=samples_per_channel/2; /*UNTIL explained, one must divide per 4 number of samples per ch */
 ...
 /*COINCIDENCE LEVEL*/
  ...
  val=gtk_range_get_value(GTK_RANGE(widget));
  delay=(int)val;
  if (delay==0) printf ("Software trigger delay is OFF \n");
  else printf("Delay is %i ns \n",delay);

  delay=delay/8;
  bd->prompt_delay_coincidence=delay;
  ...
  for (ii=0;ii<NCH;ii++){
     ...
     if (GTK_TOGGLE_BUTTON(widget)->active) 
    {
     ...
     val2=(val1/mVmax)*(pow(2.,Nbit)-1);
     val2=val2+(pow(2.,Nbit)-1);
     dacdac=(int)val2;

     printf("Channel %i, trigger thr: %f (mV)---> %i , %x \n",ii,val1,dacdac,dacdac);

     bd->ch[ii].trig_thr=dacdac;
  ...
  for (int ii=0;ii<NCH;ii++){
    ...
    if ((bd->ch[ii].enabled==1)&&(flag)){
      ...
      ret=v1720AutoSetDCOffset(bd->handle,ii,-DACoffset[ii]);
      bd->ch[ii].dac=ret;      
...
gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),1);
 ...
 //trig n over thr. Now is common to all channels
  ...
  for (ii=0;ii<NCH;ii++){
    ...
    sprintf(name,"checkbutton%i",ii);
    widget=lookup_widget(window,name);
    if (tmp) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),1);
    else gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),0);
